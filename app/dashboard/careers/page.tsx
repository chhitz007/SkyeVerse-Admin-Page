...
export default function CareersPage() {
    ...
    const filteredData = useMemo(() => {
        let filtered = data.filter((item) => {
            ...
            return (
                ...
                item.areaOfInterest?.toLowerCase().includes(term)
            )
        })
    ...
    const handleBulkDelete = async () => {
        ...
        await Promise.all(
            selectedIds.map((id) =>
                deleteDoc(doc(db, "contact_careers", id))
            )
        )
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden">
                    <div className="max-h-[70vh] overflow-y-auto">
                        ...
                        <div className={`grid ${deleteMode ? "grid-cols-4" : "grid-cols-3"} gap-4 p-4 border-b border-white/10 bg-slate-900/80 sticky top-0 text-xs uppercase tracking-wider text-slate-400`}>
                        ...
                        {filteredData.map((item) => (
                            <div
                                ...
                                className={`grid ${deleteMode ? "grid-cols-4" : "grid-cols-3"} gap-4 p-4 border-b border-white/10 hover:bg-white/10 transition cursor-pointer`}
                            >
                                {deleteMode && (
                                    <div
            ...
            {selected && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-2xl p-6 relative max-h-[85vh] overflow-y-auto">
                        ...
                        <Detail label="Area of Interest" value={selected.areaOfInterest} />
                        <Detail label="LinkedIn" value={selected.linkedin} />
                        <Detail label="Note" value={selected.note} multiline />
                    </div>
                </div>
            )}
        </div>
