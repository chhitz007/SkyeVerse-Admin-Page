...
export default function PartnershipPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const q = query(
                collection(db, "contact_partnership"),
    ...
    const filteredData = useMemo(() => {
        let filtered = data.filter((item) => {
            ...
            return (
                ...
                item.partnershipType?.toLowerCase().includes(term)
            )
        })
    ...
    const handleBulkDelete = async () => {
        ...
        await Promise.all(
            selectedIds.map((id) =>
                deleteDoc(doc(db, "contact_partnership", id))
            )
        )
            ...
            {selected && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-2xl p-6 relative max-h-[85vh] overflow-y-auto">
                        ...
                        <Detail label="Organization" value={selected.organisation} />
                        <Detail label="Partnership Type" value={selected.partnershipType} />
